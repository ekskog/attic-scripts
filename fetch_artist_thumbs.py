...
def process_folder(target_dir, use_deezer, dry_run, level):
    ...
    print(f"[*] Level: {level} | Mode: {'Deezer' if use_deezer else 'Fanart'} | {'DRY RUN' if dry_run else 'LIVE'}\n")
    ...
    try:
        for root, dirs, files in os.walk(target_dir):
            ...
            if is_artist_folder:
                ...
                if dry_run:
                    print(f"  [TEST] Artist Identified: '{clean_name}'")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Run from /mp3/ (processes all letters/artists)
  python3 fetch.py -l 1 -d /var/lib/media/music/mp3/

  # Run from inside a letter folder (e.g., /mp3/d/)
  python3 fetch.py -l 2 -d .

  # Run for a single specific artist
  python3 fetch.py -l 3 -d /var/lib/media/music/mp3/d/da_lata/

  # Always use -t first to verify folder identification!
  python3 fetch.py -l 2 -d -t .
"""
    )
    parser.add_argument("path", nargs="?", default=".", help="Target directory (default: current)")
    parser.add_argument("-d", action="store_true", help="Use Deezer API (no key required)")
    parser.add_argument("-t", action="store_true", help="Dry run: Identify folders without downloading")
    parser.add_argument("-l", type=int, choices=[1, 2, 3], required=True, 
